#### その他　書籍には書いてないこと ミスの訂正と補足情報（順次追加）
...
##### <b>・[6.4.2] 分散分析表の説明が間違っています</b> (2024.06.24)　→初版２刷では訂正済み
...
##### <b>・誤字脱字について</b> (2024.07.18)　→初版２刷では訂正済み
...
##### <b>・Jaccard距離のフリガナについて</b> (2025.10.07) 　→初版２刷では訂正済み
P189のJaccard距離の読み方は、フランス語読みをするのが正しいようですので、《ジャッカード》ではなく《ジャカール》に訂正します。

##### <b>・生成AIをコーディングではなくコード解説先生として使う</b> (2026.03.10) 　
この本ではコードをできるだけ分かりやすく書いたつもりですが、それでも伝わらない部分があるでしょう。そんな時は、生成AIに以下のようなプロンプトを書き込んで、その下に該当する（私の解説込みの）コードブロックをコピペするとよいでしょう。

プロンプト案：私は情報学系のバックグラウンドを持たない生物学系のプログラミング初心者です。以下のコードブロックに関する書籍中の解説を読んでもよく分かりませんでした。さらに詳しい解説を１行１行についてしてください。
